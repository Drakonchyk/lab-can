...
//  }
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  HAL_GPIO_TogglePin(LD2_GPIO_Port, LD2_Pin); // Блимаємо, щоб бачити, що Node A "живий"

	  // 1. Надсилаємо "ПОГАНИЙ" ID (Телеметрія GPS)
	  TxHeader.StdId = 0x120;
	  if (HAL_CAN_AddTxMessage(&hcan1, &TxHeader, TxData, &TxMailbox[0]) != HAL_OK) {
		   Error_Handler();
	  }
	  HAL_Delay(2000); // Чекаємо 2 сек

	  // 2. Надсилаємо "ДОБРИЙ" ID (Команда для FIFO0)
	  ...
	  // 3. Надсилаємо "ПОГАНИЙ" ID (Телеметрія ENV)
	  ...
	  // 4. Надсилаємо "ДОБРИЙ" ID (Heartbeat для FIFO1)
	  TxHeader.StdId = 0x081;
	  if (HAL_CAN_AddTxMessage(&hcan1, &TxHeader, TxData, &TxMailbox[0]) != HAL_OK) {
		   Error_Handler();
	  }
	  HAL_Delay(2000); // Чекаємо 2 сек
